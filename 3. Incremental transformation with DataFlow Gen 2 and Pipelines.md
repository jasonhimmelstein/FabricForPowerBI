...
Navigate back to the "Fabric Workshop xxx" workspace, select the "New" button from the ribbon, and select "More options". Next, select "Data pipeline"**.** Name it "Thermostat_file_processing"

Select the "Copy data assistant" tile on the main page.

Open the "OneLake Data hub" tab, select the "Workspace" tab, then select "Lakehouse".
...
Select the "Copy data" action, and add to canvas. then select the "Source" tab. Open the Advanced section and select the "Delete files after completion" checkbox.
